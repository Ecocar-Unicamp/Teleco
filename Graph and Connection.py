...
class tab:
    ...
    def draw(self, outline=True): # Draws the Button
        if outline: # dá pra ser melhor conceituado
            pygame.draw.rect(self.window, (0,0,0), (self.x - 2, self.y - 2, self.width + 4, self.height + 4), 0)
            pygame.draw.rect(self.window, (0,0,0), (self.close_x - 2, self.close_y - 2, self.close_width + 4, self.close_height + 4), 0)
        ...
        pygame.draw.rect(self.window, self.color, (self.x, self.y, self.width, self.height), 0)
        pygame.draw.rect(self.window, self.close_color, (self.close_x, self.close_y, self.close_width, self.close_height), 0)
        font2 = pygame.font.SysFont('freesansbold.ttf',int(1.5 * self.close_height))  ################### verificar se parametrizavel
        text = font2.render("X", True, (0, 0, 0))
        (self.window).blit(text, (self.close_x + (self.close_width / 2 - text.get_width() / 2), self.y + (self.close_height / 2 - text.get_height() / 2)))
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and button_pressed == (1,0,0):
            ...
            if connection_button.cursor_is_over(cursor_position) and not serial_COM_port:
                ...
                if serial_COM_port:
                    ...
                    list_of_tabs.append(tab(window_of_visualization, 100, 240, 100, 20, "Tab 1", True)) ################################### simplificar
            ...
            for t in range(len(list_of_tabs)):
                ...
                if list_of_tabs[t].cursor_is_over_close(cursor_position) and len(list_of_tabs) > 1:
                    ...
                    if test.selected:
                        ...
                        selected_tab = list_of_tabs[0]
                    for j in range(len(list_of_tabs) - t): ############# simplificar tanto aqui quanto na definiçao da classe
